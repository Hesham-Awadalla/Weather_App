...
namespace Weather_App
{
    class APICall
    {
        ...
        public static void LocateMe()
        {
            ...
            JObject weatherDataMain = API_Call(WetterURL, WetterUrlParam);

            string ausgabe = "Location:\n" + weatherDataMain["location"]["name"] + ", " + weatherDataMain["location"]["country"]
                           + "\n\nLocal time: " + weatherDataMain["location"]["localtime"]
                           + "\nTemperatur: " + weatherDataMain["current"]["temp_c"] + " °C | " + weatherDataMain["current"]["temp_f"] + " °F"
                           + "\nWind: " + weatherDataMain["current"]["wind_kph"] + " km/h"
            ...
            ((Image)App.Current.Resources["img"]).Source = new BitmapImage(new Uri("http:"+weatherDataMain["current"]["condition"]["icon"].ToString()));
